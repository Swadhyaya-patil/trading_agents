...
class hist_data:
    ...
    INSTRUMENT_CACHE = "data/instrument_master.json"   # cache locally
    ...
    # ── Equity historical data — the one you use ───────────────────────
    def get_eq_data(self, script_name, script_code, from_date, to_date, interval):
        params = {
            "exchange":    "NSE",
            "symboltoken": script_code,
        ...
        # ── Guard: AngelOne returns {"status": False} on errors
    ...
    def get_ltp_data(self, script_code, exchange="NFO"):
        response = self.angel_obj.getMarketData(
            "FULL", {exchange: [script_code]}
        )
